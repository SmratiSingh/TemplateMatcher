...
def normalize(img):
    ...
    # # finalImage = np.zeros(shape=(row, col))
    # # norm_img = copy.deepcopy(img)
    #
    # for i in range(0, row):
    #     for j in range(0, col):
    #         finalImage[i][j] = 255 * ((img[i][j] - minImg) / (maxImg - minImg))
    #
    ...
    for r in img:
        rList = list()
        for c in r:
           tempPix = 255 * ((c - minImg) / (maxImg - minImg))
           ...
           rList.append(tempPix)
        # if flag == 0:
        #     print("rList ", rList)
        #     flag = 1
        finalImage.append(rList)

    minImg = np.min(finalImage)
    maxImg = np.max(finalImage)
    img = finalImage
    # normimg = np.ndarray(finalImage, dtype=np.uint8)
...
def edge_magnitude(edge_x, edge_y):
    row = len(edge_x)
    col = len(edge_y[0])
    edge_mag = np.zeros(shape=(row, col))
    # edge_mag = np.ndarray(edge_mag, dtype=np.uint8)
    for i in range(0, row):
        for j in range(0, col):
            edge_mag[i][j] = np.sqrt((edge_x[i][j] ** 2) + (edge_y[i][j] ** 2))

    edge_mag = np.asarray(edge_mag, dtype=np.uint8)
    print("mag image ", edge_mag)
